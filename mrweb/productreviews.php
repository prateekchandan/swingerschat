...
<br />
	...
	<?php
        $result = mysql_query("SELECT * FROM `productreviews` WHERE `approved` = '0' ORDER BY `date` DESC");
	    ...
	    <a href=\"productremovereview.php?rid=$rid\" onclick=\"return confirm('Are you sure you want to delete this review?');\"><img src='images/trashcan.jpg'></a>
	    </td>
	    <td align=\"left\" valign=\"middle\" width='30px' style='border-bottom:3px solid #F7F7F7; border-right:1px solid #F7F7F7;'>
	    <a href=\"productapprovereview.php?rid=$rid\"><img src='images/check.jpg'></a>
	    </td>
	...
	Existing Reviews
        ...
        <table align="center" cellpadding="0px" cellspacing="5px" border="0px" style="background-color:#FFFFFF;" width="100%">
	<?php
        $result = mysql_query("SELECT * FROM `productreviews` WHERE `approved` = '1' ORDER BY `date` DESC");
        while ($r = mysql_fetch_array($result)) {
	    $rid = ($r['id']);
	    $review = stripslashes($r['review']);
	    $review = str_replace("\n", "<br />", $review);
